...
import ui.mig._
class ChoseAccounts(s: SkandiabankenSync, newAccounts: Seq[BankAccount], oldAccounts: Seq[(BankAccount, String)]) extends ui.MigPanel {
  val accountsCheckBoxes = newAccounts.map(a => new CheckBox {
    text = a.name
  })
	import ui.mig.ConstraintUnit	
  ...
  add(new Label("Nye konti oppdaget, velg de du ønsker å synkronisere:"), Span(3) >> GapBottom(0 px) >> Wrap)
	add(new ui.MigPanel("flowy, wrap 3"){
